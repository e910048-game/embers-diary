...
function isoIconHtml(itemId, fallbackCategory) {
  if (ISO_ASSETS[itemId]) return `<img class="pixelImg isoImg" src="assets/iso/${itemId}.png?v=150" alt="${itemId}">`;
...
function homeSceneHtml(state) {
  ...
  items.push(`<div class="roomCell player" id="homePlayerCell" style="left:${pos.left};top:${pos.top};z-index:${cellZ(pRow, 4)}" title="拖曳可移動位置"><div class="icon"><img class="pixelImg" src="assets/characters/${state.appearance || "char_1"}.png?v=150" alt="玩家"></div><div class="homeLabel${labelCls(pRow)}">${state.playerName || "旅人"}</div></div>`);
  if (state.companion) {
...
const avatarRow = `<div class="homeAvatarRow">
    <div class="homeAvatar">
      <div class="homeAvatarImg"><img class="pixelImg" src="assets/characters/${state.appearance || "char_1"}.png?v=150" alt="玩家"></div>
    ...
    ${state.companion ? `<div class="homeAvatar">
      <div class="homeAvatarImg"><img class="pixelImg" src="assets/characters/companion_default.png?v=150" alt="同伴"></div>
...
function itemIconHtml(itemId, type) {
  if (ICON_ASSETS[itemId]) return `<span class="icon inline"><img class="pixelImg" src="assets/icons/${itemId}.png?v=150" alt="${itemId}"></span>`;
...
function chooseStartAppearance() {
  ...
  const cardsHtml = CHARACTER_OPTIONS.map(c => `
    <div class="charCard" data-id="${c.id}">
      <img class="pixelImg" src="assets/characters/${c.id}.png?v=150" alt="${c.name}">
...
function renderBattle(message) {
  ...
  renderText(`
    <div class="enemyCard">
      <div class="icon" title="${b.enemy.icon}">${ENEMY_ASSETS[b.enemy.id] ? `<img class="pixelImg enemyImg" src="assets/enemies/${b.enemy.id}.png?v=150" alt="${b.enemy.name}">` : pixelIconSvg(b.enemy.id || b.enemy.name, "enemy")}</div>
      <div class="info">
