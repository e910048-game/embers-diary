...
function isoIconHtml(itemId, fallbackCategory) {
  if (ISO_ASSETS[itemId]) return `<img class="pixelImg isoImg" src="assets/iso/${itemId}.png?v=140" alt="${itemId}">`;
...
function homeSceneHtml(state) {
  ...
  items.push(`<div class="roomCell player" id="homePlayerCell" style="left:${pos.left};top:${pos.top};z-index:${cellZ(pRow, 4)}" title="拖曳可移動位置"><div class="icon"><img class="pixelImg" src="assets/characters/${state.appearance || "char_1"}.png?v=140" alt="玩家"></div><div class="homeLabel${labelCls(pRow)}">${state.playerName || "旅人"}</div></div>`);
  if (state.companion) {
...
const windowCls = `homeWindow ${state.phase === "night" ? "is-night" : "is-day"}${isThreatDue(state) ? " is-bloodmoon" : ""}`;
  ...
  items.push(`<div class="roomCell doorCell clickable" id="homeExploreDoorCell" style="left:50%;top:${WALL_ICON_OFFSET}px;z-index:${cellZ(0)}" title="探索門（點擊探索）"><div class="icon"><img class="pixelImg isoImg" src="assets/iso/door_explore.png?v=140" alt="探索門"></div><div class="homeLabel">探索</div></div>`);
  items.push(`<div class="roomCell doorCell clickable" id="homeGatherDoorCell" style="left:50%;top:${ROOM_H_PX - WALL_PX + WALL_ICON_OFFSET}px;z-index:${cellZ(GRID_FLOOR_ROW_MAX)}" title="採集門（點擊採集）"><div class="icon"><img class="pixelImg isoImg" src="assets/iso/door_gather.png?v=140" alt="採集門"></div><div class="homeLabel lbl-above">採集</div></div>`);
...
const avatarRow = `<div class="homeAvatarRow">
    <div class="homeAvatar">
      <div class="homeAvatarImg"><img class="pixelImg" src="assets/characters/${state.appearance || "char_1"}.png?v=140" alt="玩家"></div>
    ...
    ${state.companion ? `<div class="homeAvatar">
      <div class="homeAvatarImg"><img class="pixelImg" src="assets/characters/companion_default.png?v=140" alt="同伴"></div>
...
const ICON_ASSETS = {
  ...
  cyber_suit: 1, floor_sample: 1, food_can: 1, gaia_armor: 1, gaia_skin: 1, gaia_whip: 1,
  jacket_01: 1, merchant_token: 1, mind_eye: 1, mind_fork: 1, mind_greatsword: 1,
  mind_mirror: 1, mind_robe: 1, ocean_jacket: 1, ocean_leech: 1, ocean_mace: 1,
  ocean_pistol: 1, reinforce_blueprint: 1, scrap: 1, scrap_plating: 1, serum_atk: 1,
  serum_stamina: 1, serum_vit: 1, tesla_battery: 1, vest_01: 1, water_bottle: 1, wedding_ring: 1,
...
function itemIconHtml(itemId, type) {
  if (ICON_ASSETS[itemId]) return `<span class="icon inline"><img class="pixelImg" src="assets/icons/${itemId}.png?v=140" alt="${itemId}"></span>`;
...
function chooseStartAppearance() {
  ...
  const cardsHtml = CHARACTER_OPTIONS.map(c => `
    <div class="charCard" data-id="${c.id}">
      <img class="pixelImg" src="assets/characters/${c.id}.png?v=140" alt="${c.name}">
...
function renderBattle(message) {
  ...
  renderText(`
    <div class="enemyCard">
      <div class="icon" title="${b.enemy.icon}">${ENEMY_ASSETS[b.enemy.id] ? `<img class="pixelImg enemyImg" src="assets/enemies/${b.enemy.id}.png?v=140" alt="${b.enemy.name}">` : pixelIconSvg(b.enemy.id || b.enemy.name, "enemy")}</div>
      <div class="info">
